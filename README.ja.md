...
- **macOS（Sonoma / macOS 14 以前）** — **Markwell** を右クリック（または
  Control キーを押しながらクリック）して「**開く**」を選び、ダイアログで
  もう一度「**開く**」をクリックします。macOS は選択を覚えるので、必要なのは
  最初の一度だけです。
- **macOS（Sequoia / macOS 15 以降）** — 右クリックの方法は使えません。
  **Markwell** を一度開き（ブロックされます）、「**システム設定 → プライバシー
  とセキュリティ**」で「**このまま開く**」をクリックします。
...
気に入らなければ、いつでも削除できます。Markwell はバックグラウンドに何も
インストールしません。常駐サービスも、アカウントも、レジストリへの書き込みも、
ネットワーク接続も一切ありません。

- **macOS / Windows** — （解凍した）**Markwell** アプリをゴミ箱に移すだけです。
  あとには何も残りません。
- **Python パッケージ** — `pipx uninstall markwell`（または `pip uninstall markwell`）。

ライブラリと保存コピーは別のフォルダ（既定では `~/Markwell`、「**設定**」で
確認できます）にあるため、アンインストールしてもそれらには触れません。
きれいに消したい場合は、そのフォルダと、設定を保存する `~/.markwell/` も
削除してください。どちらもあなたが完全に管理できる、ふつうのフォルダです。
...
います — hi@markwell.page。Markwell は無料のオープンソースで、これからもずっと
無料です。あなたの読書を残す役に立ったなら、リポジトリにスターを付けたり、
引用カードをシェアしたり、[コーヒーを奢って ☕](https://ko-fi.com/erictu)いただけたら嬉しいです。

## 謝辞

Markwell が成り立つのは **Kobo** のおかげです。ハイライトを、USB でデバイスから
直接読める標準的な SQLite データベースという開かれた形式で保存し、読者にも
開発者にもやさしい姿勢をとってくれています。Kobo と
[@kobolabs](https://github.com/kobolabs) に感謝します。
